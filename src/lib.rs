...
mod storage_types;
use constants::{
    DEFAULT_COUNTER_VALUE, DEFAULT_TTL_LEDGERS, LEDGERS_PER_DAY, TOKEN_DECIMALS,
    USER_COUNT_INCREMENT, ZERO_HASH_BYTES,
};
...
pub enum ContractError {
    ...
    InvalidMigration = 11,
    /// The provided archetype is not in the admin-managed allowlist. (code 12)
    InvalidArchetype = 12,
}
...
impl StellarWrapContract {
    ...
    pub fn claim_wrap(
        ...
        e.storage()
            .persistent()
            .extend_ttl(&claim_key, DEFAULT_TTL_LEDGERS, DEFAULT_TTL_LEDGERS);
    ...
    fn persist_wrap_record(
        ...
        e.storage()
            .persistent()
            .set(&count_key, &(current_count + USER_COUNT_INCREMENT));
        e.storage()
            .persistent()
        ...
        let current_latest: u64 = e
            ...
            .persistent()
            .get(&latest_key)
            .unwrap_or(DEFAULT_COUNTER_VALUE.into());
    ...
    pub fn balance_of(e: Env, id: Address) -> i128 {
        ...
        e.storage()
            .persistent()
            .get::<_, u32>(&count_key)
            .unwrap_or(DEFAULT_COUNTER_VALUE) as i128
    }
    ...
    pub fn extend_ttl(e: Env, user: Address, period: u64) {
        ...
        if e.storage().persistent().has(&wrap_key) {
            e.storage()
                .persistent()
                .extend_ttl(&wrap_key, LEDGERS_PER_DAY, DEFAULT_TTL_LEDGERS);
        }
        ...
        if e.storage().persistent().has(&count_key) {
            e.storage()
                .persistent()
                .extend_ttl(&count_key, LEDGERS_PER_DAY, DEFAULT_TTL_LEDGERS);
        }
        ...
        if e.storage().persistent().has(&latest_key) {
            e.storage()
                .persistent()
                .extend_ttl(&latest_key, LEDGERS_PER_DAY, DEFAULT_TTL_LEDGERS);
        }
    ...
    /// Add an allowed archetype. Admin-only.
    ...
    /// Remove an allowed archetype. Admin-only.
    ...
    /// Return the current admin-managed archetype allowlist.
