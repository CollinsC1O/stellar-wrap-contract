...
fn test_allowed_archetypes_initialized() {
    ...
    assert!(allowed.contains(&symbol_short!("arch")));
    assert!(allowed.contains(&symbol_short!("architect")));
    assert!(allowed.contains(&symbol_short!("soroban")));
...
fn test_invalid_archetype_rejected() {
    ...
    let hash = BytesN::from_array(&env, &[82u8; 32]);
    let sig = sign_payload(
        &env,
        &signing_key,
        &contract_id,
...
fn test_admin_can_add_and_remove_archetype() {
    ...
    let period = 2026u64;
    let hash = BytesN::from_array(&env, &[83u8; 32]);
    let sig = sign_payload(
        &env,
        &signing_key,
        &contract_id,
...
fn test_wrap_record_display_is_single_line_with_hash_preview() {
    ...
    let record = WrapRecord {
        timestamp: 123,
        data_hash: BytesN::from_array(&env, &[42u8; 32]),
    ...
    assert!(output.contains("period: 2025"));
